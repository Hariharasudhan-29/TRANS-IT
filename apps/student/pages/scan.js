...
import { getFirestore, collection, addDoc, query, where, getDocs, doc, getDoc } from 'firebase/firestore'; // Keep functionality
...
export default function ScanPage() {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (!snapshot.empty) {
                setError("You are already boarded on this bus!");
            ...
            await addDoc(collection(db, 'boardings'), {
                ...
                status: 'Boarded',
                scannedAt: Date.now(),
                parentNotified: !!parentPhoneNumber
            });
                ...
                try {
                    await fetch('/api/send-sms', {
                        ...
                        body: JSON.stringify({
                            ...
                            message: `[TRANS-IT] Your child ${user.displayName || 'Student'} has boarded Bus ${busNum} safely at ${new Date().toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit' })}.`
                        })
                    });
                    alert(`✅ Boarded Successfully!\n\n📲 SMS sent to parent: ${parentPhoneNumber}`);
                } catch (smsError) {
            ...
            console.log(`Boarding recorded for bus: ${busNum}`);
