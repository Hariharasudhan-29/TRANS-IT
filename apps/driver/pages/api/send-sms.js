    ...
    try {
        if (twilioSid && twilioToken && twilioFrom) {
            ...
            const response = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilioSid}/Messages.json`, {
                ...
                body: new URLSearchParams({ 'To': formattedNumber, 'From': twilioFrom, 'Body': message })
            });
            ...
            const response = await fetch('https://www.fast2sms.com/dev/bulkV2', {
                ...
                body: JSON.stringify({ route: "q", message, language: "english", flash: 0, numbers: cleanNumber.replace('+91', '') })
            });
            const data = await response.json();
            ...
            console.warn('⚠️ No SMS API keys in .env — simulated success.');
            return res.status(200).json({ success: true, simulated: true, message: "SMS simulated." });
        }
    } catch (error) {
